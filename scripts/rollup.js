...
function getConfig(options, index, bundles) {
  const lastBundle = index === bundles.length - 1;
  ...
  return {
    ...
    plugins: [
      ...
      !production && lastBundle && copy({
        targets: [{ src: 'packages/playerx/dist', dest: 'public' }]
      }),

      // In dev mode, call `npm run start` once
      // the bundle has been generated
      !production && lastBundle && serve(),

      // Watch the `public` directory and refresh the
      // browser on changes when not in production
      !production && lastBundle && livereload('public')
